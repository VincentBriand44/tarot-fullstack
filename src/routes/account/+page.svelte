...
<div class="flex flex-col items-center">
	<div class="p-8 rounded-full bg-slate-900 w-min">
		<Icon icon={'mi-user'} size={5} />
	</div>
	<h2 class="text-xl uppercase bold">
		{user?.name}
		<span class="font-bold text-red-500">
			{user?.role !== 'USER' ? ` (${user?.role})` : ''}
		</span>
	</h2>
	<h3 class="italic font-light text-slate-400">@{user?.username.toLowerCase()}</h3>
		...
		<div class="w-full px-4 py-2 bg-slate-900 rounded-xl">
			...
			<ul class="flex flex-col gap-2 py-2">
				{#each lastGames as game}
					<li class="relative flex justify-between px-4 py-2 bg-slate-800 rounded-xl">
						<p>
							Partie jouer le
						...
						<a href={`/games/${game?.id}`}>
							<button class="px-2 text-blue-500 rounded-lg bg-slate-900 hover:bg-slate-950">
								Accèder au détails
							</button>
						</a>
					</li>
